...
<br/>
    ...
    <textarea class="form-control" id="notification1" rows="5" required>'.$notification1.'</textarea>

    <br/>

    <label>Option for notification *</label>
    <input type="hidden" id="req_token" value="'.$_SESSION['csrftoken'].'">
    <select class="form-control" id="notification1_mode" required>
...
?>
    ...
    <script type="text/javascript">
     ...
     var formData = {
      notification1: $("#notification1").val(),
      notification1_mode: $("#notification1_mode").val(),
      req_token: $("#req_token").val(),
    };
    $.LoadingOverlay("show");
    $.ajax({
      url:'customer/notification1',
      method:'POST',
      dataType:'json',
      data:formData,
