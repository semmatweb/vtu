...
<br/>
    ...
    <textarea class="form-control" id="notification3" rows="5" required>'.$notification1.'</textarea>

    <br/>

    <label>Option for notification *</label>
    <input type="hidden" id="req_token" value="'.$_SESSION['csrftoken'].'">
    <select class="form-control" id="notification3_mode" required>
...
?>
    ...
    <script type="text/javascript">
     ...
     var formData = {
      notification3: $("#notification3").val(),
      notification3_mode: $("#notification3_mode").val(),
      req_token: $("#req_token").val(),
    };
    $.LoadingOverlay("show");
    $.ajax({
      url:'customer/notification3',
      method:'POST',
      dataType:'json',
      data:formData,
