...
<h3 class="f_s_25 f_w_700 dark_text">All Pending Users</h3>
...
$getUser=mysqli_query($con, "SELECT * FROM users WHERE status='0' ORDER BY id DESC LIMIT 2000");

if (mysqli_num_rows($getUser)<1){

$message = '
<div class="alert alert-danger">
<h4>No Pending Users Sign Up Yet !!!</h4>
</div>';
...
if ($viewUsers['status']==0){

$todobtn='<button class="btn btn-success" id="'.$viewUsers['id'].'" onclick="approveUser(this.id)">Approve</button>';
}

else {

$todobtn='<button class="btn btn-danger" id="'.$viewUsers['id'].'" onclick="blockUser(this.id)">Block</button>';
}
...
<script type="text/javascript">
 ...
 $.LoadingOverlay("show");
    $.ajax({
      url:'customer/approve_user?data='+id,
      dataType:"Json",
