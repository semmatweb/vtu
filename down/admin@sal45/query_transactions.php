...
$ddaa = mysqli_query($db,"SELECT * FROM mytransaction WHERE username='$userid' OR email='$userid' OR trx='$userid' OR descr LIKE '%$userid%' ORDER BY id DESC LIMIT 20000");
  ...
  <div>
   ...
   <form name="sendrequest" id ="sendrequest" method="POST"> 
                                ...
                                <div class="form-group">
                                  <label for="exampleInputEmail1">Email/Username/Ref No./Mobile No. <span style="color:red">*</span></label>
                                    <input type="text" class="form-control" name="userid" id="userid" required autocomplete="off">
                                </div>
                                
                                <button type="submit" name="submit" class="btn btn-warning btn-block">QUERY</button>
                            </form>
  </div>
  <br/>
  <br/>
